...
@section('subheader', 'Agrega una nueva categoría al menú.')
...
<div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden max-w-3xl mx-auto">
    <form action="{{ route('categorias.guardar') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
        @csrf
            ...
            <div class="col-span-2 sm:col-span-1">
                ...
                <div class="mt-1 flex rounded-md shadow-sm">
                    <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-amber-500 sm:text-lg">
                        <i class="fas fa-icons" id="icon-preview"></i>
                    </span>
                    <select name="icono" id="icono" onchange="document.getElementById('icon-preview').className = this.value || 'fas fa-icons'"
                        ...
                        <option value="fas fa-star" {{ old('icono') == 'fas fa-star' ? 'selected' : '' }}>⭐ Estrellita (Por defecto)</option>
                        <option value="fas fa-bread-slice" {{ old('icono') == 'fas fa-bread-slice' ? 'selected' : '' }}>🍞 Horneados / Pan</option>
                        <option value="fas fa-hamburger" {{ old('icono') == 'fas fa-hamburger' ? 'selected' : '' }}>🍔 Salados / Comida Rápida</option>
                        <option value="fas fa-cookie" {{ old('icono') == 'fas fa-cookie' ? 'selected' : '' }}>🍪 Galletas / Dulces</option>
                        <option value="fas fa-birthday-cake" {{ old('icono') == 'fas fa-birthday-cake' ? 'selected' : '' }}>🎂 Tortas / Postres</option>
                        <option value="fas fa-coffee" {{ old('icono') == 'fas fa-coffee' ? 'selected' : '' }}>☕ Café / Calientes</option>
                        <option value="fas fa-glass-whiskey" {{ old('icono') == 'fas fa-glass-whiskey' ? 'selected' : '' }}>🥤 Bebidas / Refrescos</option>
                        <option value="fas fa-utensils" {{ old('icono') == 'fas fa-utensils' ? 'selected' : '' }}>🍽️ Platos Fuertes</option>
                        <option value="fas fa-pizza-slice" {{ old('icono') == 'fas fa-pizza-slice' ? 'selected' : '' }}>🍕 Pizzas</option>
                        <option value="fas fa-ice-cream" {{ old('icono') == 'fas fa-ice-cream' ? 'selected' : '' }}>🍦 Helados</option>
                        <option value="fas fa-leaf" {{ old('icono') == 'fas fa-leaf' ? 'selected' : '' }}>🌿 Saludable / Natural</option>
                    </select>
                </div>
        ...
        <div class="pt-4 border-t border-gray-200 flex justify-end">
            <button type="submit" class="inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                <i class="fas fa-save mr-2 mt-0.5"></i> Guardar Categoría
            </button>
        </div>
    </form>
</div>
